...
static JSONValue *parse_array(const char **p) {
    ...
    value->u.array.items = malloc(value->u.array.capacity * sizeof(JSONValue *));
    ...
    while (1) {
        ...
        if (value->u.array.count >= value->u.array.capacity) {
            ...
            JSONValue **new_items = realloc(value->u.array.items, new_capacity * sizeof(JSONValue *));
            if (!new_items) {
...
static JSONValue *parse_object(const char **p) {
    ...
    value->u.object.keys = malloc(value->u.object.capacity * sizeof(char *));
    value->u.object.values = malloc(value->u.object.capacity * sizeof(JSONValue *));
    ...
    while (1) {
        ...
        free(key_val);  // We only need the string
        ...
        if (value->u.object.count >= value->u.object.capacity) {
            ...
            char **new_keys = realloc(value->u.object.keys, new_capacity * sizeof(char *));
            JSONValue **new_values = realloc(value->u.object.values, new_capacity * sizeof(JSONValue *));
...
int main(void) {
    ...
    */
    ...
    char *ser_json = malloc(ser_capacity);
    if (!ser_json) {
